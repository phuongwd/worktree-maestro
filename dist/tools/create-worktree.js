...
import { z } from 'zod';
import { getRepoRoot, getRepoName, getDefaultBranch, branchExists, createWorktree as gitCreateWorktree, } from '../utils/git.js';
...
function generateBranchName(ticket, name, prefix) {
...
function generateWorktreeDirName(repoName, ticket, name) {
    const sanitizedName = sanitizeName(name);
    if (ticket) {
        const sanitizedTicket = ticket.toUpperCase().replace(/[^A-Z0-9-]/g, '-');
